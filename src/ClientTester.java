...
public class ClientTester {
    ...
    public static void prefixMerge(Client [] list1, Client [] list2, Client[] result) {
        ...
        for (int i = 0; resCount < result.length; i++)                                  //goes through list 1 for the length of result
        {
            for (int j = 0; j < list2.length && resCount < result.length; j++)          //goes through list 2 completely
            {
                for (int k = 0; k < resCount; k++)                                      //checks to make sure value isn't already in result
                {
                    if (result[k].compareClient(list2[j]) == 0)
                        {j++;}
                }
                if(list1[i].compareClient(list2[j]) > 0)                                //determines which of the two compared goes in result first (alphabetically or numerical)
                { result[resCount] = list2[j];
                    resCount++; }
            }
            if (resCount < result.length)                                               //puts the other value compared above into result if there's room
                { result[resCount] = list1[i];                                          //(after comparing every value of list2)
                resCount++; }
        }
    }
